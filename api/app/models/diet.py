...
class User(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_users_approval_status", "is_approved"
        ),  # For filtering pending users
    )

    # Relationships
    diets = relationship(
        "WeeklyDiet", back_populates="user", cascade="all, delete-orphan"
    )
...
class WeeklyDiet(Base):
    ...
    meals = relationship(
        "Meal", back_populates="weekly_diet", cascade="all, delete-orphan"
    )
    grocery_list = relationship(
        "GroceryList",
        back_populates="weekly_diet",
        uselist=False,
        cascade="all, delete-orphan",
    )
...
class Meal(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, nullable=False)
    weekly_diet_id: Mapped[str] = mapped_column(
        String, ForeignKey("weekly_diets.id", ondelete="CASCADE"), nullable=False
    )
    meal_type: Mapped[TipoPasto] = mapped_column(SQLEnum(TipoPasto), nullable=False)
    day: Mapped[GiornoSettimana] = mapped_column(
        SQLEnum(GiornoSettimana), nullable=False
    )
    time: Mapped[str] = mapped_column(String, nullable=True)
    recipe: Mapped[str] = mapped_column(String, nullable=True)
    ingredienti: Mapped[str] = mapped_column(
...
class MealIngredient(Base):
    ...
    ingredient_id: Mapped[str] = mapped_column(
        String, ForeignKey("ingredients.id", ondelete="CASCADE"), nullable=False
    )
...
class GroceryList(Base):
    ...
    # Table constraints
    __table_args__ = (Index("idx_grocery_lists_weekly_diet_id", "weekly_diet_id"),)
...
class GroceryListItem(Base):
    ...
    grocery_list_id: Mapped[str] = mapped_column(
        String, ForeignKey("grocery_lists.id", ondelete="CASCADE"), nullable=False
    )
    ingredient_id: Mapped[str] = mapped_column(
        String, ForeignKey("ingredients.id", ondelete="CASCADE"), nullable=False
    )
...
class SavedRecipe(Base):
    ...
    )  # e.g., "Porridge di avena con banana"
    recipe_instructions: Mapped[str] = mapped_column(
        String, nullable=False
...
class UserSettings(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "age IS NULL OR (age > 0 AND age < 150)", name="chk_user_settings_valid_age"
        ),
        CheckConstraint(
            "weight IS NULL OR weight > 0", name="chk_user_settings_positive_weight"
        ),
        CheckConstraint(
