...
class DatabaseManager:
    ...
    def _create_engine(self) -> None:
        ...
        logger.info(
            f"  - Max overflow: {settings.database_max_overflow if pool_class != NullPool else 'N/A'}"
        )
    ...
    def _get_optimal_pool_class(self) -> type:
        ...
        Returns:
            ...
            QueuePool for traditional deployments
        """
        ...
        # or in development for simpler debugging
        if is_transaction_pooler or settings.is_development:
            logger.info("Using NullPool (no client-side pooling)")
            return NullPool

        # Use QueuePool for direct connections and production environments
    ...
    def _build_connect_args(self) -> dict[str, Any]:
        ...
        connect_args = {
            ...
            "application_name": f"api-diet-{settings.environment}",
            "connect_timeout": 10,  # Connection timeout in seconds
        }

        # Add keep-alive settings for long-running connections
        ...
        # Add statement timeout for query safety via options
        command_timeout = getattr(settings, "database_command_timeout", 30)
        if command_timeout:
            connect_args["options"] = f"-c statement_timeout={command_timeout * 1000}"
